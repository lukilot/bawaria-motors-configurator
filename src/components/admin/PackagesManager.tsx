...
export function PackagesManager() {
    ...
    const handleSavePackage = async (pkg: ServicePackage) => {
        // ID is auto-generated by DB default, but upsert needs it if editing.
        // If adding new, we don't send ID.
        // Actually, we can just send the whole object.
        const { error } = await supabase.from('service_packages').upsert(pkg, { onConflict: 'code, vehicle_type' });
        if (error) {
    ...
    return (
        <div className="p-8 max-w-7xl mx-auto">
            ...
            {loading ? (
                ...
                <div className="grid gap-4">
                    {packages.filter(p => p.type === activeTab).map(pkg => (
                        <div key={pkg.id} className="bg-white border border-gray-200 rounded-sm p-4 hover:shadow-sm transition-shadow">
            ...
            {isPriceModalOpen && editingPkg && (
                <PriceModal
                    pkg={editingPkg}
                    allPrices={prices.filter(p => p.package_id === editingPkg.id)}
...
function PriceModal({ pkg, allPrices, onClose, onUpdate }: { pkg: ServicePackage, allPrices: any[], onClose: () => void, onUpdate: () => void }) {
    ...
    const handleDelete = async (series: string) => {
        ...
        await supabase.from('service_prices').delete().match({ package_id: pkg.id, series_code: series });
        onUpdate();
    };
