...
async function run() {
    ...
    if (!groups || !cars) throw new Error("Could not fetch data");
    ...
    const activeGroupsWithImages = groups.filter(g => g.images && g.images.length > 0 && groupToVins[g.id] && groupToVins[g.id].length > 0);
    
    let pendingCarsCount = 0;
    emptyTargets.forEach(g => pendingCarsCount += groupToVins[g.id].length);

    console.log(`REMAINING Pending Setup (No Photos) Cars: ${pendingCarsCount}`);
    console.log(`Total Active Groups (with photos): ${activeGroupsWithImages.length}`);
}
run();
