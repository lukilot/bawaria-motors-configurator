...
async function run() {
    console.log("Searching for ALL groups with model 41HK...");
    const groups = await fetchAll('product_groups', '*');
    const g41hk = groups.filter(g => g.model_code === '41HK');
    
    for (const g of g41hk) {
        console.log(`Group ${g.id}: Photos=${g.images?.length || 0}, Sig=${g.signature}`);
    }
}
run();
